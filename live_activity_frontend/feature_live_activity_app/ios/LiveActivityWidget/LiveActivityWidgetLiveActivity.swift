        ...
        ActivityConfiguration(for: LiveActivityAttributes.self) { context in
            ...
            DynamicIsland {
                DynamicIslandExpandedRegion(.leading) {
                    if context.state.carArriveProgress < 100 {
                        Text("Pickup in").foregroundColor(.white).padding(
                            EdgeInsets(top: 4, leading: 12, bottom: 0, trailing: 12))
                    } else {
                        Text("Driver Arrives").foregroundColor(.white).padding(
                            EdgeInsets(top: 4, leading: 12, bottom: 0, trailing: 12))
                    }
                ...
                DynamicIslandExpandedRegion(.trailing) {
                    if context.state.carArriveProgress < 100 {
                        Text("\(context.state.minutesToArrive) min").foregroundColor(.white)
                            .padding(EdgeInsets(top: 4, leading: 12, bottom: 0, trailing: 12))
                    } else {
                        Text("Enjoy your trip").foregroundColor(.white).padding(
                            EdgeInsets(top: 4, leading: 12, bottom: 0, trailing: 12))
                    }
                ...
                DynamicIslandExpandedRegion(.bottom) {
                    if context.state.carArriveProgress < 100 {
                        ProgressBarWithCar(progress: context.state.carArriveProgress).padding(
                            EdgeInsets(top: 0, leading: 42, bottom: 0, trailing: 42))
                    } else {
                        ZStack(
                            alignment: Alignment(horizontal: .center, vertical: .center),
                            content: {
                                Circle().fill(Color.blue).frame(width: 52, height: 52)
                                Image("flag").resizable().frame(width: 42, height: 42).scaledToFit()
                ...
                if context.state.carArriveProgress < 100 {
                    Text("Pickup in").foregroundColor(.white)
                } else {
                    Text("Driver Arrives").foregroundColor(.white)
                }
                ...
                if context.state.carArriveProgress < 100 {
                    ...
                    ZStack(
                        ...
                        content: {
                            ...
                            Image("flag").resizable().frame(width: 25, height: 25).scaledToFit()
                                .aspectRatio(contentMode: .fit)
                        })
                }
            } minimal: {
                if context.state.carArriveProgress < 100 {
                    Text("Driver on the way").foregroundColor(.white)
                } else {
                    Text("Driver Arrives").foregroundColor(.white)
                }
            }
...
extension LiveActivityAttributes.ContentState {
    fileprivate static var model1: LiveActivityAttributes.ContentState {
        LiveActivityAttributes.ContentState(
            driverCode: "Matheus", carModel: "Virtus", minutesToArrive: 10, carArriveProgress: 30)
    }

    fileprivate static var model2: LiveActivityAttributes.ContentState {
        LiveActivityAttributes.ContentState(
            driverCode: "Matheus", carModel: "Virtus", minutesToArrive: 10, carArriveProgress: 30)
    }
}
            ...
            HStack(
                ...
                content: {
                    VStack(
                        alignment: .leading,
                        content: {
                            Text("Pickup in \(context.state.minutesToArrive) min").semiBold20()
                    ...
                    ZStack(
                        ...
                        content: {
                            ...
                            Image("driver").resizable().frame(width: 32, height: 32).scaledToFit()
                                .aspectRatio(contentMode: .fit)
                        })
            ...
            HStack(
                ...
                content: {
                    VStack(
                        alignment: .leading,
                        content: {
                            Text("Your Driver Arrives").semiBold20().padding(
                                EdgeInsets(top: 4, leading: 0, bottom: 0, trailing: 2))
        ...
        VStack {
            ZStack(alignment: .leading) {
                ProgressView(value: Double(self.progress), total: 100)
                    .progressViewStyle(LinearProgressViewStyle(tint: .white)).background(
