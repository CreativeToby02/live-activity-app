        ...
        case "startNotifications":
          if let args = call.arguments as? [String: Any] {
            if #available(iOS 16.2, *) {
              ...
              LiveNotificationManager.shared.showNotification(progress: progress, minutesToDelivery: minutes)
            }
            result("Notification displayed")
          } else {
            result(FlutterError(code: "INVALID_ARGUMENTS", message: "Missing progress/minutesToDelivery", details: nil))
          }
        case "updateNotifications":
          if let args = call.arguments as? [String: Any] {
            if #available(iOS 16.2, *) {
              ...
              let progress = args["progress"] as? Int ?? 0
              let minutes = args["minutesToDelivery"] as? Int ?? 0
              LiveNotificationManager.shared.updateNotification(progress: progress, minutesToDelivery: minutes)
            }
